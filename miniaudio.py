...
GeneratorTypes = Union[
    PlaybackCallbackGeneratorType,
    CaptureCallbackGeneratorType,
    DuplexCallbackGeneratorType,
]
...
class SoundFileInfo:
    ...
    def __init__(
        ...
        self.sample_format_name = ffi.string(
            lib.ma_get_format_name(sample_format.value)
        ).decode()
    ...
    def __str__(self) -> str:
        ...
        return (
            "<{clazz}: '{name}' {fileformatdisplay} {nchannels} ch, {sample_rate} hz, {sample_format.name}, "
            "{num_frames} frames={duration:.2f} sec.>".format(
                clazz=self.__class__.__name__,
                fileformatdisplay=fileformatdisplay,
                **(vars(self)),
            )
        )
...
def vorbis_read_file(filename: str) -> DecodedSoundFile:
    ...
    filenamebytes = _get_filename_bytes(filename)
    with ffi.new("int *") as channels, ffi.new("int *") as sample_rate, ffi.new(
        "short **"
    ) as output:
        num_frames = lib.stb_vorbis_decode_filename(
            filenamebytes, channels, sample_rate, output
        )
        if num_frames <= 0:
        ...
        try:
            ...
            return DecodedSoundFile(
                filename, channels[0], sample_rate[0], SampleFormat.SIGNED16, samples
            )
        finally:
            lib.free(output[0])
...
def vorbis_read(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("int *") as channels, ffi.new("int *") as sample_rate, ffi.new(
        ...
        num_samples = lib.stb_vorbis_decode_memory(
            data, len(data), channels, sample_rate, output
        )
        if num_samples <= 0:
        ...
        try:
            ...
            samples.frombytes(buffer)
            return DecodedSoundFile(
                "<memory>", channels[0], sample_rate[0], SampleFormat.SIGNED16, samples
            )
        finally:
            lib.free(output[0])


def vorbis_stream_file(
    filename: str, seek_frame: int = 0
) -> Generator[array.array, None, None]:
    ...
    with ffi.new("int *") as error:
        ...
        try:
            info = lib.stb_vorbis_get_info(vorbis)
            with ffi.new("short[]", 4096 * info.channels) as decode_buffer1, ffi.new(
                "short[]", 4096 * info.channels
            ) as decode_buffer2:
                ...
                while True:
                    ...
                    if num_samples1 + num_samples2 <= 0:
                        break
                    buffer = ffi.buffer(
                        decode_buffer1, num_samples1 * 2 * info.channels
                    )
                    samples = _create_int_array(2)
                    samples.frombytes(buffer)
                    if num_samples2 > 0:
                        buffer = ffi.buffer(
                            decode_buffer2, num_samples2 * 2 * info.channels
                        )
                        samples.frombytes(buffer)
                    yield samples
        finally:
...
def flac_get_file_info(filename: str) -> SoundFileInfo:
    ...
    if sys.platform == "win32":
        ...
        flac = lib.ma_dr_flac_open_file_w(filenamebytesw, ffi.NULL)
    else:
        filenamebytes = _get_filename_bytes(filename)
        flac = lib.ma_dr_flac_open_file(filenamebytes, ffi.NULL)
    if not flac:
        raise DecodeError("could not open/decode file")
    try:
...
def flac_read_file_s32(filename: str) -> DecodedSoundFile:
    ...
    filenamebytes = _get_filename_bytes(filename)
    with ffi.new("unsigned int *") as channels, ffi.new(
        "unsigned int *"
    ) as sample_rate, ffi.new("ma_dr_flac_uint64 *") as num_frames:
        memory = lib.ma_dr_flac_open_file_and_read_pcm_frames_s32(
            filenamebytes, channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode file")
        try:
            ...
            samples.frombytes(buffer)
            return DecodedSoundFile(
                filename, channels[0], sample_rate[0], SampleFormat.SIGNED32, samples
            )
        finally:
            lib.ma_dr_flac_free(memory, ffi.NULL)
...
def flac_read_file_s16(filename: str) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_flac_open_file_and_read_pcm_frames_s16(
            filenamebytes, channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode file")
...
def flac_read_file_f32(filename: str) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_flac_open_file_and_read_pcm_frames_f32(
            filenamebytes, channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode file")
        try:
            ...
            samples.frombytes(buffer)
            return DecodedSoundFile(
                filename, channels[0], sample_rate[0], SampleFormat.FLOAT32, samples
            )
        finally:
            lib.ma_dr_flac_free(memory, ffi.NULL)
...
def flac_read_s32(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_flac_open_memory_and_read_pcm_frames_s32(
            data, len(data), channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode data")
        try:
            ...
            samples.frombytes(buffer)
            return DecodedSoundFile(
                "<memory>", channels[0], sample_rate[0], SampleFormat.SIGNED32, samples
            )
        finally:
            lib.ma_dr_flac_free(memory, ffi.NULL)
...
def flac_read_s16(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_flac_open_memory_and_read_pcm_frames_s16(
            data, len(data), channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode data")
...
def flac_read_f32(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_flac_open_memory_and_read_pcm_frames_f32(
            data, len(data), channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode data")
        try:
            ...
            samples.frombytes(buffer)
            return DecodedSoundFile(
                "<memory>", channels[0], sample_rate[0], SampleFormat.FLOAT32, samples
            )
        finally:
            lib.ma_dr_flac_free(memory, ffi.NULL)


def flac_stream_file(
    filename: str, frames_to_read: int = 1024, seek_frame: int = 0
) -> Generator[array.array, None, None]:
    ...
    if not flac:
        raise DecodeError("could not open/decode file")
    if seek_frame > 0:
        result = lib.ma_dr_flac_seek_to_pcm_frame(flac, seek_frame)
        if result <= 0:
            raise DecodeError("can't seek")
    try:
        with ffi.new(
            "ma_dr_flac_int16[]", frames_to_read * flac.channels
        ) as decodebuffer:
            buf_ptr = ffi.cast("ma_dr_flac_int16 *", decodebuffer)
            while True:
                num_samples = lib.ma_dr_flac_read_pcm_frames_s16(
                    flac, frames_to_read, buf_ptr
                )
                if num_samples <= 0:
                    break
...
def mp3_get_file_info(filename: str) -> SoundFileInfo:
    ...
    with ffi.new("ma_dr_mp3 *") as mp3:
        if sys.platform == "win32":
            filenamebytesw = _get_filename_bytes_w(filename)
            result = lib.ma_dr_mp3_init_file_w(mp3, filenamebytesw, ffi.NULL)
        else:
            filenamebytes = _get_filename_bytes(filename)
            result = lib.ma_dr_mp3_init_file(mp3, filenamebytes, ffi.NULL)
        if not result:
            raise DecodeError("could not open/decode file")
        try:
            num_frames = lib.ma_dr_mp3_get_pcm_frame_count(mp3)
...
def mp3_read_file_f32(filename: str) -> DecodedSoundFile:
    ...
    filenamebytes = _get_filename_bytes(filename)
    with ffi.new("ma_dr_mp3_config *") as config, ffi.new(
        "ma_dr_mp3_uint64 *"
    ) as num_frames:
        memory = lib.ma_dr_mp3_open_file_and_read_pcm_frames_f32(
            filenamebytes, config, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode file")
        try:
            ...
            lib.ma_dr_mp3_free(memory, ffi.NULL)
...
def mp3_read_file_s16(filename: str) -> DecodedSoundFile:
    ...
    with ffi.new("ma_dr_mp3_config *") as config, ffi.new(
        ...
        memory = lib.ma_dr_mp3_open_file_and_read_pcm_frames_s16(
            filenamebytes, config, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode file")
...
def mp3_read_f32(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("ma_dr_mp3_config *") as config, ffi.new(
        ...
        memory = lib.ma_dr_mp3_open_memory_and_read_pcm_frames_f32(
            data, len(data), config, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode data")
        try:
...
def mp3_read_s16(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("ma_dr_mp3_config *") as config, ffi.new(
        ...
        memory = lib.ma_dr_mp3_open_memory_and_read_pcm_frames_s16(
            data, len(data), config, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode data")
...
def mp3_stream_file(
    ...
    with ffi.new("ma_dr_mp3 *") as mp3:
        ...
        if not result:
            raise DecodeError("could not open/decode file")
        if seek_frame > 0:
            result = lib.ma_dr_mp3_seek_to_pcm_frame(mp3, seek_frame)
            if result <= 0:
                raise DecodeError("can't seek")
        try:
            with ffi.new(
                "ma_dr_mp3_int16[]", frames_to_read * mp3.channels
            ) as decodebuffer:
                buf_ptr = ffi.cast("ma_dr_mp3_int16 *", decodebuffer)
                while True:
                    num_samples = lib.ma_dr_mp3_read_pcm_frames_s16(
...
def wav_get_file_info(filename: str) -> SoundFileInfo:
    ...
    with ffi.new("ma_dr_wav*") as wav:
        if sys.platform == "win32":
            # Windows filepaths with non-ascii characters fail to open using fopen, cast filenames to wchar_t*
            filenamebytesw = _get_filename_bytes_w(filename)
            result = lib.ma_dr_wav_init_file_w(wav, filenamebytesw, ffi.NULL)
        else:
            filenamebytes = _get_filename_bytes(filename)
            result = lib.ma_dr_wav_init_file(wav, filenamebytes, ffi.NULL)
        if not result:
            raise DecodeError("could not open/decode file")
        try:
            duration = wav.totalPCMFrameCount / wav.sampleRate
            sample_width = wav.bitsPerSample // 8
            is_float = wav.translatedFormatTag == lib.MA_DR_WAVE_FORMAT_IEEE_FLOAT
...
def wav_read_file_s32(filename: str) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        "unsigned int *"
    ) as sample_rate, ffi.new("ma_dr_wav_uint64 *") as num_frames:
        memory = lib.ma_dr_wav_open_file_and_read_pcm_frames_s32(
            filenamebytes, channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode file")
        try:
            ...
            lib.ma_dr_wav_free(memory, ffi.NULL)
...
def wav_read_file_s16(filename: str) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_wav_open_file_and_read_pcm_frames_s16(
            filenamebytes, channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode file")
...
def wav_read_file_f32(filename: str) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_wav_open_file_and_read_pcm_frames_f32(
            filenamebytes, channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode file")
...
def wav_read_s32(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_wav_open_memory_and_read_pcm_frames_s32(
            data, len(data), channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode data")
...
def wav_read_s16(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_wav_open_memory_and_read_pcm_frames_s16(
            data, len(data), channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode data")
...
def wav_read_f32(data: bytes) -> DecodedSoundFile:
    ...
    with ffi.new("unsigned int *") as channels, ffi.new(
        ...
        memory = lib.ma_dr_wav_open_memory_and_read_pcm_frames_f32(
            data, len(data), channels, sample_rate, num_frames, ffi.NULL
        )
        if not memory:
            raise DecodeError("cannot load/decode data")
...
def wav_stream_file(
    ...
    with ffi.new("ma_dr_wav*") as wav:
        ...
        if not result:
            raise DecodeError("could not open/decode file")
        if seek_frame > 0:
            result = lib.ma_dr_wav_seek_to_pcm_frame(wav, seek_frame)
            if result <= 0:
                raise DecodeError("can't seek")
        try:
            with ffi.new(
                "ma_dr_wav_int16[]", frames_to_read * wav.channels
            ) as decodebuffer:
                buf_ptr = ffi.cast("ma_dr_wav_int16 *", decodebuffer)
                while True:
                    num_samples = lib.ma_dr_wav_read_pcm_frames_s16(
...
def wav_write_file(filename: str, sound: DecodedSoundFile) -> None:
    ...
    with ffi.new("ma_dr_wav_data_format*") as fmt, ffi.new("ma_dr_wav*") as pwav:
        fmt.container = lib.ma_dr_wav_container_riff
        fmt.format = sound.sub_format or lib.MA_DR_WAVE_FORMAT_PCM
        ...
        if not lib.ma_dr_wav_init_file_write_sequential(
            pwav, filename_bytes, fmt, sound.num_frames * sound.nchannels, ffi.NULL
        ):
            raise IOError("can't open file for writing")
        try:
            lib.ma_dr_wav_write_pcm_frames(
                pwav, sound.num_frames, sound.samples.tobytes()
            )
        finally:
            lib.ma_dr_wav_uninit(pwav)
...
class Devices:
    ...
    def __init__(self, backends: Optional[List[Backend]] = None) -> None:
        ...
        self.backend = ffi.string(
            lib.ma_get_backend_name(self._context[0].backend)
        ).decode()

    def get_playbacks(self) -> List[Dict[str, Any]]:
        """Get a list of playback devices and some details about them"""
        with ffi.new("ma_device_info**") as playback_infos, ffi.new(
            "ma_uint32*"
        ) as playback_count:
            result = lib.ma_context_get_devices(
                self._context, playback_infos, playback_count, ffi.NULL, ffi.NULL
            )
            if result != lib.MA_SUCCESS:
            ...
            for i in range(playback_count[0]):
                ...
                dev_id = ffi.new(
                    "ma_device_id *", ma_device_info.id
                )  # copy the id memory
                info = {
    ...
    def get_captures(self) -> List[Dict[str, Any]]:
        ...
        with ffi.new("ma_device_info**") as capture_infos, ffi.new(
            "ma_uint32*"
        ) as capture_count:
            result = lib.ma_context_get_devices(
                self._context, ffi.NULL, ffi.NULL, capture_infos, capture_count
            )
            if result != lib.MA_SUCCESS:
                raise MiniaudioError("cannot get device infos", result)
            devs = []
    ...
    def _get_info(
        self, device_type: DeviceType, device_info: ffi.CData
    ) -> Dict[str, Any]:
        # obtain detailed info about the device
        result = lib.ma_context_get_device_info(
            self._context,
            device_type.value,
            ffi.addressof(device_info.id),
            ffi.addressof(device_info),
        )
        if result != lib.MA_SUCCESS:
        ...
        for fmt in range(device_info.nativeDataFormatCount):
            ...
            formats.append(
                {
                    "format": ffi.string(
                        lib.ma_get_format_name(data_format.format)
                    ).decode(),
...
def _array_proto_from_format(sampleformat: SampleFormat) -> array.array:
    ...
    raise MiniaudioError(
        "the requested sample format can not be used directly: "
        + sampleformat.name
        + " (convert it first)"
    )
...
def decode_file(
    ...
    with ffi.new("ma_uint64 *") as frames, ffi.new("void **") as memory:
        decoder_config = lib.ma_decoder_config_init(
            output_format.value, nchannels, sample_rate
        )
        decoder_config.ditherMode = dither.value
        result = lib.ma_decode_file(
            filenamebytes, ffi.addressof(decoder_config), frames, memory
        )
        if result != lib.MA_SUCCESS:
        ...
        return DecodedSoundFile(
            filename, nchannels, sample_rate, output_format, samples
        )
...
def decode(
    ...
    with ffi.new("ma_uint64 *") as frames, ffi.new("void **") as memory:
        ...
        decoder_config.ditherMode = dither.value
        result = lib.ma_decode_memory(
            data, len(data), ffi.addressof(decoder_config), frames, memory
        )
        if result != lib.MA_SUCCESS:
        ...
        lib.ma_free(memory[0], ffi.NULL)
        return DecodedSoundFile(
            "<memory>", nchannels, sample_rate, output_format, samples
        )
        ...
        with ffi.new(
            ...
            while True:
                if want_frames > allocated_buffer_frames:
                    raise MiniaudioError(
                        "wanted to read more frames than storage was allocated for ({} vs {})".format(
                            want_frames, allocated_buffer_frames
                        )
                    )
                num_frames = 0
                with ffi.new("ma_uint64 *") as frames_read:
                    try:
                        result = lib.ma_decoder_read_pcm_frames(
                            decoder, buf_ptr, want_frames, frames_read
                        )
                ...
                if source and source.error_in_readcallback:
                    ...
                    ) from source.error_in_readcallback
...
def stream_file(
    ...
    if sys.platform == "win32":
        filenamebytesw = _get_filename_bytes_w(filename)
        result = lib.ma_decoder_init_file_w(
            filenamebytesw, ffi.addressof(decoder_config), decoder
        )
    else:
        filenamebytes = _get_filename_bytes(filename)
        result = lib.ma_decoder_init_file(
            filenamebytes, ffi.addressof(decoder_config), decoder
        )
    if result != lib.MA_SUCCESS:
    ...
    g = _samples_stream_generator(
        frames_to_read, nchannels, output_format, decoder, None
    )
    dummy = next(g)
    assert len(dummy) == 0
    return g
...
def stream_memory(
    ...
    result = lib.ma_decoder_init_memory(
        data, len(data), ffi.addressof(decoder_config), decoder
    )
    if result != lib.MA_SUCCESS:
        raise DecodeError("failed to init decoder", result)
    g = _samples_stream_generator(
        frames_to_read, nchannels, output_format, decoder, data
    )
    dummy = next(g)
    assert len(dummy) == 0
    return g
...
def stream_raw_pcm_memory(
    pcmdata: Union[array.array, memoryview, bytes],
    nchannels: int,
    sample_width: int,
    frames_to_read: int = 4096,
) -> PlaybackCallbackGeneratorType:
    """
...
class IceCastClient(StreamableSource):
    ...
    def __init__(
        self,
        url: str,
        update_stream_title: Callable[["IceCastClient", str], None] = None,
        ...
        self._download_thread = threading.Thread(
            target=self._download_stream, daemon=True
        )
...
def stream_any(
    ...
    result = lib.ma_decoder_init(
        lib._internal_decoder_read_callback,
        lib._internal_decoder_seek_callback,
        source.ffi_handle,
        ffi.addressof(decoder_config),
        decoder,
    )
    if result != lib.MA_SUCCESS:
        raise DecodeError("failed to init decoder", result)
    ...
    g = _samples_stream_generator(
        frames_to_read, nchannels, output_format, decoder, None, on_close
    )
    dummy = next(g)
    assert len(dummy) == 0
    return g
...
@ffi.def_extern()
def _internal_decoder_read_callback(
    decoder: ffi.CData, output: ffi.CData, num_bytes: int, bytes_read: ffi.CData
) -> int:
    bytes_read[0] = 0
...
@ffi.def_extern()
def _internal_decoder_seek_callback(
    decoder: ffi.CData, offset: int, seek_origin: int
) -> int:
...
def convert_frames(
    ...
    sample_width = width_from_format(to_fmt)
    output_frame_count = lib.ma_calculate_frame_count_after_resampling(
        to_samplerate, from_samplerate, num_frames
    )
...
@ffi.def_extern()
def _internal_data_callback(
    device: ffi.CData, output: ffi.CData, input: ffi.CData, framecount: int
) -> None:
...
class AbstractDevice:
    ...
    def _make_context(
        ...
        if app_name:
            ...
            context_config.pulse.pApplicationName = ffi.from_buffer(
        ...
        if backends:
            ...
            for i, b in enumerate(backends):
                backends_mem[i] = b.value
            result = lib.ma_context_init(
                backends_mem, len(backends), ffi.addressof(context_config), context
            )
            if result != lib.MA_SUCCESS:
                raise MiniaudioError("cannot init context", result)
        else:
            result = lib.ma_context_init(
                ffi.NULL, 0, ffi.addressof(context_config), context
            )
            if result != lib.MA_SUCCESS:
                raise MiniaudioError("cannot init context", result)
        return context
...
class CaptureDevice(AbstractDevice):
    ...
    def __init__(
        self,
        input_format: SampleFormat = SampleFormat.SIGNED16,
        nchannels: int = 2,
        sample_rate: int = 44100,
        buffersize_msec: int = 200,
        device_id: Union[ffi.CData, None] = None,
        callback_periods: int = 0,
        ...
        result = lib.ma_device_init(
            self._context, ffi.addressof(self._devconfig), self._device
        )
        if result != lib.MA_SUCCESS:
        ...
        self.backend = ffi.string(
            lib.ma_get_backend_name(self._device.pContext.backend)
        ).decode()
    ...
    def _data_callback(
        self, device: ffi.CData, output: ffi.CData, input: ffi.CData, framecount: int
    ) -> None:
        if self.callback_generator:
        ...
        if self.callback_generator:
            ...
            if samples_bytes:
                if len(samples_bytes) > framecount * self.sample_width * self.nchannels:
                    self.callback_generator = None
                    raise MiniaudioError(
                        "number of frames from callback exceeds maximum"
                    )
...
class DuplexStream(AbstractDevice):
    ...
    def __init__(
        self,
        playback_format: SampleFormat = SampleFormat.SIGNED16,
        playback_channels: int = 2,
        capture_format: SampleFormat = SampleFormat.SIGNED16,
        capture_channels: int = 2,
        sample_rate: int = 44100,
        buffersize_msec: int = 200,
        playback_device_id: Union[ffi.CData, None] = None,
        capture_device_id: Union[ffi.CData, None] = None,
        callback_periods: int = 0,
        backends: Optional[List[Backend]] = None,
        thread_prio: ThreadPriority = ThreadPriority.HIGHEST,
        app_name: str = "",
...
def _bytes_from_generator_samples(
    samples: Union[array.array, memoryview, bytes],
) -> bytes:
...
class WavFileReadStream(io.RawIOBase):
    ...
    def __init__(
        self,
        pcm_sample_gen: PlaybackCallbackGeneratorType,
        sample_rate: int,
        nchannels: int,
        output_format: SampleFormat,
        max_frames: int = 0,
        ...
        self.bytes_done = 0
        # create WAVE header
        fmt = ffi.new("ma_dr_wav_data_format*")
        fmt.container = lib.ma_dr_wav_container_riff
        fmt.format = lib.MA_DR_WAVE_FORMAT_PCM
        fmt.channels = nchannels
        fmt.sampleRate = sample_rate
        fmt.bitsPerSample = self.sample_width * 8
        data = ffi.new("void**")
        datasize = ffi.new("size_t *")
        pwav = ffi.new("ma_dr_wav*")
        if max_frames > 0:
            lib.ma_dr_wav_init_memory_write_sequential(
                pwav, data, datasize, fmt, max_frames * nchannels, ffi.NULL
            )
        else:
            lib.ma_dr_wav_init_memory_write(pwav, data, datasize, fmt, ffi.NULL)
        lib.ma_dr_wav_uninit(pwav)
        self.buffered = bytes(ffi.buffer(data[0], datasize[0]))
        lib.ma_dr_wav_free(data[0], ffi.NULL)
